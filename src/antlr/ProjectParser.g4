...
proge
    : (statement | NEWLINE)* EOF  #progeRoot
    ;
...
testlist_star_expr
    : (test | NEWLINE) (COMMA (test | NEWLINE))* (COMMA)?
    | test comp_for
    ;
...
dictmaker
    : (test COLON test | NEWLINE)+ (COMMA (test COLON test | NEWLINE)*)* (COMMA)?
...
suite : simple_stmt | INDENT statement+ DEDENT ;

exprlist : (test | NEWLINE) (COMMA (test | NEWLINE)*)* (COMMA)? ;
testlist : (test | NEWLINE) (COMMA (test | NEWLINE)*)* (COMMA)? ;
